...
def main():
    ...
    parser = argparse.ArgumentParser(description="Monopoly Game Server")
    parser.add_argument(
        "--host", "-H",
        default=None,
        help=f"Server host address (default: {settings.HOST}, or SERVER_HOST env var)"
    )
    parser.add_argument(
        "--port", "-p",
        type=int,
        default=None,
        help=f"Server port (default: {settings.PORT}, or SERVER_PORT env var)"
    )
    parser.add_argument(
        "--db-path", "-d",
        default=None,
        help=f"Database path (default: {settings.DATABASE_PATH}, or DATABASE_PATH env var)"
    )
    parser.add_argument(
        "--log-level", "-l",
        default=None,
        choices=["DEBUG", "INFO", "WARNING", "ERROR"],
        help=f"Log level (default: {settings.LOG_LEVEL}, or LOG_LEVEL env var)"
    )
    ...
    Config.from_args(args.host, args.port, args.db_path)
