...
class Game:
    ...
    def _move_player(
        ...
        expected_position = (old_position + spaces) % BOARD_SIZE
        
        passed_go = player.move_forward(spaces)
        
        # Log movement for debugging
        logger.debug(
            f"MOVE: {player.name} moved {spaces} spaces: {old_position} -> {player.position} "
            f"(expected {expected_position}, dice={dice_result.die1}+{dice_result.die2})"
        )
        
        # Verify movement was correct
        if player.position != expected_position:
            logger.error(
                f"MOVEMENT BUG DETECTED! Player {player.name} (id={player.id}): "
                f"was at {old_position}, moved {spaces} spaces, "
                f"expected {expected_position}, but ended at {player.position}"
            )
    ...
    def get_state_for_player(self, player_id: str) -> dict:
        ...
        """
        # Log positions being sent for debugging
        positions = {pid: self.players[pid].position for pid in self.player_order if pid in self.players}
        logger.debug(f"STATE for {player_id}: positions={positions}, turn={self.turn_number}")
